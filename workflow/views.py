...
class WorkFlowTriggerViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [permissions.IsAuthenticated, ]
    lower_range = openapi.Parameter('lower_range', openapi.IN_QUERY,
                                    type=openapi.TYPE_STRING)
    higher_range = openapi.Parameter('higher_range', openapi.IN_QUERY,
                                     type=openapi.TYPE_STRING)
    ...
    @action(detail=False, methods=['get'], url_path="search")
    @swagger_auto_schema(manual_parameters=[lower_range, higher_range])
    def filter_workflow_trigger_with_time(self, request, *args, **kwargs):
        '%Y-%m-%d %H:%M:%S'
        lower_range = datetime.datetime.strptime(request.query_params.get('lower_range'),
                                        '%Y-%m-%d %H:%M:%S')
        higher_range = datetime.datetime.strptime(request.query_params.get('higher_range'),
                                         '%Y-%m-%d %H:%M:%S')
        workflow_id = self.kwargs.get('workflow_id')
